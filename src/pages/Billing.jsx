...
export default function Billing() {
  const { settings, connected, isAdmin, profile } = useApp()
  const cur = settings.currency_symbol || '৳'
  ...
  const load = async () => {
    ...
    if (!isAdmin && profile?.resident_id) {
      feeData = feeData.filter(item => item.resident_id === profile.resident_id)
    }
    setFees(feeData)
    setResidents(r.data || [])
    setLoading(false)
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
        ...
        {connected && isAdmin && <button className="btn btn-accent" onClick={() => setShowAdd(true)}><SvgIcon d={ICONS.plus} size={14} /> Add Fee</button>}
      </div>
